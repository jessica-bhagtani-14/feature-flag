...
class EvaluationService {
  async evaluateFlag(appId, flagKey, context = {}) {
    ...
    try {
      ...
      if (!flagWithRules.enabled) {
        ...
        await this.recordEvaluation(flagWithRules.id, appId, false, context, startTime);
      ...
      // Record evaluation in analytics
      await this.recordEvaluation(flagWithRules.id, appId, result.enabled, context, startTime);
  ...
  async recordEvaluation(flagId, appId, enabled, context, startTime) {
    try {
      ...
      // Record evaluation using analytics service
      await analyticsService.recordEvaluation(evaluationData);
      ...
      // Don't throw error for logging failures - analytics should be non-blocking
